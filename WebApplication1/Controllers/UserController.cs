...
namespace WebApplication1.Controllers
{
    public class UserController : Controller
    {
        ...
        public async Task<IActionResult> Login(TblUser user)
        {
            ...
            var userdetail = await _db.TblUsers.Where(a => a.Id == user.Id).FirstOrDefaultAsync();
            var assignedRole = await _db.TblUserRoles.Where(a => a.UserId == user.Id).FirstOrDefaultAsync();
            if (user.Role != null)
            {
